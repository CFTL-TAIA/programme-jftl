import { deleteAdminMedia } from './admin-media.mjs';
import {
  createStoredResource,
  deleteStoredResource,
  getDataStorageLabel,
  isPostgresDataStorageEnabled,
  loadDatabaseData,
...
export async function createResource(resourceType, payload) {
  ...
  const items = await loadResource(resourceType);
  ...
  if (!isPostgresDataStorageEnabled()) {
    throw new Error(`La variable taia_bdd_DATABASE_URL ou DATABASE_URL est obligatoire. Stockage actuel: ${getDataStorageLabel()}.`);
  }

  await createStoredResource(resourceType, normalizedPayload);
...
export async function updateResource(resourceType, payload) {
  ...
  await updateStoredResource(resourceType, normalizedPayload);
  return normalizedPayload;
}
...
export async function deleteResource(resourceType, id) {
  ...
  const items = await loadResource(resourceType);
  const deletedItem = items.find((item) => item[definition.idField] === id) || null;
  ...
  await deleteStoredResource(resourceType, id);

  const mediaReference = resourceType === 'speaker'
    ? deletedItem.photo
    : resourceType === 'entreprise'
      ? deletedItem.logo
      : '';

  if (mediaReference) {
    await deleteAdminMedia(mediaReference);
  }
...
export function sendServerError(response, message) {
  const normalizedMessage = String(message || 'Erreur interne.').includes('Precondition failed: ETag mismatch.')
    ? 'Le stockage distant est encore en cours de synchronisation. Attendez quelques secondes puis recommencez.'
    : message;
  ...
  sendJson(
    response,
    {
      error: {
        code: 'SERVER_ERROR',
        message: normalizedMessage || 'Erreur interne.'
      }
    },
    500
  );
